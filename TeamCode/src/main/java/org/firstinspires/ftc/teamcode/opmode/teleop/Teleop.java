...
public class Teleop extends OpMode {
    ...
    public void checkCarousel() {
        if (player1.b && player1.b != preValueB) {
            carouselTimer = null;
            if (!redCarouselActive && !blueCarouselActive && robot.carousel.getPower() == 0) {
        ...
        if (player1.x && player1.x != preValueX) {
           carouselTimer = null;
            if (!blueCarouselActive && !redCarouselActive && robot.carousel.getPower() == 0) {
        ...
        preValueX = player1.x;
        if(player1.right_stick_button || player2.right_stick_button)
        {
        ...
        else if(!redCarouselActive && !blueCarouselActive) {robot.carousel.setPower(0);speed = curspeed;}
        ...
        if (player2.right_bumper && player2.right_bumper != preValueRBumper2) {
            if ((carouselmodifier + maxCarouselSpeed) >= 1) {
                carouselmodifier = 1-maxCarouselSpeed;
            } else {
    ...
    public void checkDrive() {
        ...
        if(robot.slider.getCurrentPosition() > 2300) {speedCap = true; curspeed = 1; speed = .25;}//speed cap enabled when capstone is up
        else speedCap = false;
    ...
    public void printTelemetry() {
        if (practiceMode) {
            ...
            double roundedModifier = ((int)(carouselmodifier*10)) /10.0;
            ...
            telemetry.addLine(" Carousel speed = " + (maxCarouselSpeed + roundedModifier));
            telemetry.addLine("Carousel speed modifier = " + roundedModifier);
            telemetry.addLine();
